    ...
    public function new(
        ...
        if (!$communaute || !$communaute->canPost($this->getUser())) {
            throw $this->createAccessDeniedException(
                'Vous devez être le créateur ou un membre invité pour commenter dans cette communauté.'
            );
        }
    ...
    public function edit(
        ...
        if ($request->isMethod('POST')) {
            ...
            if (isset($data['contenu']) && trim($data['contenu']) !== '') {
                ...
                // 🔥 IA MODERATION lors de modification aussi
                if (!$aiModeration->isClean($data['contenu'])) {
                    $this->addFlash('error', 'Modification refusée : contenu inapproprié détecté.');
                    return $this->redirectToRoute('app_communaute_show', [
